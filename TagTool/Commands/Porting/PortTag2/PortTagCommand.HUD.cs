...
namespace TagTool.Commands.Porting
{
    partial class PortTag2Command
    {
        ...
        private ChudDefinition ConvertChudDefinition(ChudDefinition chudDefinition)
        {
            for (int hudWidgetIndex = 0; hudWidgetIndex < chudDefinition.HudWidgets.Count; hudWidgetIndex++)
            {
                ...
                for (int bitmapWidgetIndex = 0; bitmapWidgetIndex < chudDefinition.HudWidgets[hudWidgetIndex].BitmapWidgets.Count; bitmapWidgetIndex++)
                {
                    ...
                    if (bitmapwidgetname.Contains("waypoint_light") && blam_cache_version == CacheVersion.Halo3ODST)
                    {
                    ...
                    if (chudDefinition.HudWidgets[hudWidgetIndex].PlacementData.Count == 0
                        || (blam_cache_version == CacheVersion.Halo3ODST && widgetname.Contains("vitality_meter"))
                        || (blam_cache_version == CacheVersion.Halo3ODST && widgetname.Contains("compass")))
                    {
		...
		private ChudGlobalsDefinition ConvertChudGlobalsDefinition(CacheFile blam_cache, ChudGlobalsDefinition H3Definition)
		{
                ...
                for (int hudSoundsIndex = 0; hudSoundsIndex < H3Definition.HudGlobals[hudGlobalsIndex].HudSounds.Count; hudSoundsIndex++)
                {
                    ...
                    if (blam_cache_version == CacheVersion.Halo3Retail)
                    {
                        ...
                        if (H3snd.SpartanSound != null)
                        {
                            ...
                            spartanBiped.Sound = PortTagReference(blam_cache, H3snd.SpartanSound.Index);
                        ...
                        if (H3snd.EliteSound != null)
                        {
                            ...
                            eliteBiped.Sound = PortTagReference(blam_cache, H3snd.SpartanSound.Index);
