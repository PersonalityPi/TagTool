...
namespace TagTool.Commands.Porting
{
    partial class PortTag2Command
    {
        ...
        private Sound ConvertSound(CacheFile blam_cache, List<ConvertTagTask> tag_conversion_tasks, Stream cacheStream, Dictionary<ResourceLocation, Stream> resourceStreams, Sound sound)
        {
            if (BlamSoundGestalt == null)
                BlamSoundGestalt = PortingContextFactory.LoadSoundGestalt(CacheContext, ref blam_cache);
            ...
            for (int u = 0; u < sound.SoundReference.PitchRangeCount; u++)
            {
                ...
                pitchRange.ImportName = (StringId)ConvertData(blam_cache, tag_conversion_tasks, cacheStream, resourceStreams, BlamSoundGestalt.ImportNames[pitchRange.ImportNameIndex].Name, null, null);
                ...
                var resourceData = blam_cache.GetSoundRaw(sound.SoundReference.ZoneAssetHandle, xmaFileSize);
                ...
                for (i = 0; i < permutationCount; i++)
                {
                    ...
                    permutation.ImportName = (StringId)ConvertData(blam_cache, tag_conversion_tasks, cacheStream, resourceStreams, BlamSoundGestalt.ImportNames[permutation.ImportNameIndex].Name, null, null);
        ...
        private SoundLooping ConvertSoundLooping(CacheFile blam_cache, SoundLooping soundLooping)
        {
            if (BlamSoundGestalt == null)
                BlamSoundGestalt = PortingContextFactory.LoadSoundGestalt(CacheContext, ref blam_cache);
        ...
        private Dialogue ConvertDialogue(CacheFile blam_cache, Stream cacheStream, Dialogue dialogue)
        {
            if (BlamSoundGestalt == null)
                BlamSoundGestalt = PortingContextFactory.LoadSoundGestalt(CacheContext, ref blam_cache);
