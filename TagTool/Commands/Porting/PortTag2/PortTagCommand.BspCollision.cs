...
namespace TagTool.Commands.Porting
{
    ...
    {
        private List<CollisionMoppCode.Datum> ConvertCollisionMoppData(CacheFile blam_cache, List<CollisionMoppCode.Datum> moppData)
        {
        ...
        private PageableResource ConvertStructureBspTagResources(CacheFile blam_cache, ScenarioStructureBsp bsp, Dictionary<ResourceLocation, Stream> resourceStreams)
        {
            //
            ...
            var resourceEntry = blam_cache.ResourceGestalt.TagResources[bsp.ZoneAssetIndex3 & ushort.MaxValue];
            ...
            bsp.CollisionBspResource.Resource.DefinitionData = blam_cache.ResourceGestalt.FixupInformation.Skip(resourceEntry.FixupInformationOffset).Take(resourceEntry.FixupInformationLength).ToArray();
            ...
            using (var definitionWriter = new EndianWriter(definitionStream, EndianFormat.BigEndian))
            {
                ...
                resourceDefinition = blam_cache.Deserializer.Deserialize<StructureBspTagResources>(dataContext);
            ...
            var resourceData = blam_cache.GetRawFromID(bsp.ZoneAssetIndex3);
            ...
            using (var resourceWriter = new EndianWriter(dataStream, EndianFormat.LittleEndian))
            {
                ...
                foreach (var collisionBsp in resourceDefinition.CollisionBsps)
                {
                    ...
                    for (var i = 0; i < collisionBsp.Bsp3dNodes.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<CollisionGeometry.Bsp3dNode>(dataContext));
                    ...
                    for (var i = 0; i < collisionBsp.Planes.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<CollisionGeometry.Plane>(dataContext));
                    ...
                    for (var i = 0; i < collisionBsp.Leaves.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<CollisionGeometry.Leaf>(dataContext));
                    ...
                    for (var i = 0; i < collisionBsp.Bsp2dReferences.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<CollisionGeometry.Bsp2dReference>(dataContext));
                    ...
                    for (var i = 0; i < collisionBsp.Bsp2dNodes.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<CollisionGeometry.Bsp2dNode>(dataContext));
                    ...
                    for (var i = 0; i < collisionBsp.Surfaces.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<CollisionGeometry.Surface>(dataContext));
                    ...
                    for (var i = 0; i < collisionBsp.Edges.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<CollisionGeometry.Edge>(dataContext));
                    ...
                    for (var i = 0; i < collisionBsp.Vertices.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<CollisionGeometry.Vertex>(dataContext));
                }
                ...
                foreach (var largeCollisionBsp in resourceDefinition.LargeCollisionBsps)
                {
                    ...
                    for (var i = 0; i < largeCollisionBsp.Bsp3dNodes.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<StructureBspTagResources.LargeCollisionBspBlock.Bsp3dNode>(dataContext));
                    ...
                    for (var i = 0; i < largeCollisionBsp.Bsp2dReferences.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<StructureBspTagResources.LargeCollisionBspBlock.Bsp2dReference>(dataContext));
                    ...
                    for (var i = 0; i < largeCollisionBsp.Bsp2dNodes.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<StructureBspTagResources.LargeCollisionBspBlock.Bsp2dNode>(dataContext));
                    ...
                    {
                        var surface = blam_cache.Deserializer.Deserialize<StructureBspTagResources.LargeCollisionBspBlock.Surface>(dataContext);
                    ...
                    for (var i = 0; i < largeCollisionBsp.Edges.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<StructureBspTagResources.LargeCollisionBspBlock.Edge>(dataContext));
                    ...
                    for (var i = 0; i < largeCollisionBsp.Vertices.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<StructureBspTagResources.LargeCollisionBspBlock.Vertex>(dataContext));
                }
                ...
                foreach (var instance in resourceDefinition.InstancedGeometry)
                {
                    ...
                    {
                        var surface = blam_cache.Deserializer.Deserialize<CollisionGeometry.Surface>(dataContext);
                        // surface.Material = PortGlobalMaterialIndex(CacheContext, BlamCache, surface.Material);
                        CacheContext.Serializer.Serialize(dataContext, surface);
                    }
                    ...
                    foreach (var moppCode in instance.CollisionMoppCodes)
                    {
                        ...
                        if (blam_cache_version < CacheVersion.Halo3ODST)
                            moppData = ConvertCollisionMoppData(blam_cache, moppData);
                    ...
                    for (var i = 0; i < instance.Unknown1.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<StructureBspTagResources.InstancedGeometryBlock.Unknown1Block>(dataContext));
                    ...
                    {
                        var element = blam_cache.Deserializer.Deserialize<StructureBspTagResources.InstancedGeometryBlock.Unknown2Block>(dataContext);
                        if (blam_cache_version <= CacheVersion.Halo3ODST)
                        {
                    ...
                    for (var i = 0; i < instance.Unknown3.Count; i++)
                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<StructureBspTagResources.InstancedGeometryBlock.Unknown3Block>(dataContext));
                }
