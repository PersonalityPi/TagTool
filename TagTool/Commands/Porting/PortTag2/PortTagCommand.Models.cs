...
namespace TagTool.Commands.Porting
{
    partial class PortTag2Command
    {
        ...
        private object ConvertGen2RenderModel(CacheFile blam_cache, CachedTagInstance edTag, RenderModel mode, Dictionary<ResourceLocation, Stream> resourceStreams)
        {
            foreach (var section in mode.Sections)
            {
                ...
                using (var stream = new MemoryStream(blam_cache.GetRawFromID(section.BlockOffset, section.BlockSize)))
                using (var reader = new EndianReader(stream, blam_cache.Reader.Format))
                using (var writer = new EndianWriter(stream, blam_cache.Reader.Format))
                {
                    ...
                    var mesh = blam_cache.Deserializer.Deserialize<Mesh>(dataContext);
                    ...
                    foreach (var resource in section.Resources)
                    {
                        ...
                        var elementStream = new VertexElementStream(stream, blam_cache.Reader.Format);
            ...
            foreach (var section in mode.PrtInfo)
            {
                ...
                using (var writer = new EndianWriter(stream, blam_cache.Reader.Format))
                {
                    ...
                    var dataContext = new DataSerializationContext(reader);
                    var rawPcaDatum = blam_cache.Deserializer.Deserialize<RenderModel.PrtInfoBlock.RawPcaDatum>(dataContext);
