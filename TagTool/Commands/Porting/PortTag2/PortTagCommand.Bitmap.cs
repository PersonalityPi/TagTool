...
namespace TagTool.Commands.Porting
{
    ...
    {
        private Bitmap ConvertBitmap(CacheFile blam_cache, Bitmap bitmap, Dictionary<ResourceLocation, Stream> resourceStreams)
        {
            bitmap.Flags = BitmapRuntimeFlags.UsingTagInteropAndTagResource;
            bitmap.UnknownB4 = 0;

            if (blam_cache_version == CacheVersion.HaloReach)
            {
            ...
            for (int i = 0; i < bitmap.Images.Count(); i++)
            {
                ...
                {
#endif
                    var resource = ConvertBlamBitmap(blam_cache, bitmap, resourceStreams, i);
                    if (resource == null)
                        return null;
        ...
        private PageableResource ConvertBlamBitmap(CacheFile blam_cache, Bitmap bitmap, Dictionary<ResourceLocation, Stream> resourceStreams, int imageIndex)
        {
            ...
            try
            {
                ...
                if (blamBitmap.Image.XboxFlags.HasFlag(BitmapFlagsXbox.UseInterleavedTextures))
                {
                    ...
                    byte[] totalRaw = blam_cache.GetRawFromID(rawID, (interleavedIndex + 1) * rawSize);
                ...
                else
                {
                    ...
                    raw = blam_cache.GetRawFromID(rawID, rawSize);
                    if (raw == null)
            ...
            {
                raw = ConvertBlamBitmapData(blam_cache, raw, blamBitmap);
            }
            ...
            using (var dataStream = new MemoryStream(raw))
            {
                ...
                {
                    resourceStreams[ResourceLocation.Textures] = Flags.HasFlag(PortingFlags.Memory) ?
                        new MemoryStream() :
        ...
        private byte[] ConvertBlamBitmapData(CacheFile blam_cache, byte[] raw, BlamBitmap blamBitmap)
        {
            ...
            switch (blamBitmap.Format)
            {
                ...
                case BitmapFormat.Dxt3aMono:
                    raw = DxtDecoder.DecodeBitmap(raw, blamBitmap.Image, blamBitmap.VirtualWidth, blamBitmap.VirtualHeight, blam_cache_version);
