...
namespace TagTool.Commands.Porting
{
    ...
    {
        public ModelAnimationGraph ConvertModelAnimationGraph(CacheFile blam_cache, Stream cacheStream, Dictionary<ResourceLocation, Stream> resourceStreams, ModelAnimationGraph definition)
        {
            if (blam_cache.ResourceGestalt == null)
                blam_cache.LoadResourceTags();
            ...
            {
                var resourceEntry = blam_cache.ResourceGestalt.TagResources[group.ZoneAssetDatumIndex & ushort.MaxValue];
                ...
                group.Resource = new PageableResource
                {
                    ...
                    Resource = new TagResource
                    {
                        ...
                        DefinitionData = blam_cache.ResourceGestalt.FixupInformation.Skip(resourceEntry.FixupInformationOffset).Take(resourceEntry.FixupInformationLength).ToArray(),
                ...
                if (group.Resource.Resource.DefinitionData.Length != 0)
                {
                    ...
                    using (var definitionWriter = new EndianWriter(definitionStream, EndianFormat.BigEndian))
                    {
                        ...
                        resourceDefinition.Add(blam_cache.Deserializer.Deserialize<ModelAnimationTagResource>(dataContext));
                    }
                }
            }
            ...
            foreach (var group in definition.ResourceGroups)
            {
                ...
                var resourceData = blam_cache.GetRawFromID(group.ZoneAssetDatumIndex);
                ...
                using (var resourceWriter = new EndianWriter(dataStream, EndianFormat.LittleEndian))
                {
                    ...
                    foreach (var member in resourceDefinition[resDefIndex].GroupMembers)
                    {
                        ...
                        if ((byte)member.BaseHeader != 0)
                        {
                            ...
                            codec = blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.Codec>(dataContext);


                            CacheContext.Serializer.Serialize(dataContext, codec);

                            var Format1 = blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.Format1>(dataContext);
                            ...
                            for (int i = 0; i < codec.RotationNodeCount; i++)
                                CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.RotationFrame>(dataContext));
                            ...
                            for (int i = 0; i < codec.PositionNodeCount; i++)
                                CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.PositionFrame>(dataContext));
                            ...
                            for (int i = 0; i < codec.ScaleNodeCount; i++)
                                CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.ScaleFrame>(dataContext));
                        ...
                        switch (codec.AnimationCodec)
                        {
                            ...
                            case ModelAnimationTagResource.AnimationCompressionFormats.Type3: // should merge with type1
                                var header = blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.Format1>(dataContext);
                            ...
                            case ModelAnimationTagResource.AnimationCompressionFormats.Type7:
                                ...
                                var overlay = blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.Overlay>(dataContext);
                                ...
                                for (int i = 0; i < codec.RotationNodeCount; i++)
                                {
                                    frameInfo = blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.FrameInfo>(dataContext);
                                ...
                                for (int i = 0; i < RotationFrameCount; i++)
                                    if (codec.AnimationCodec == ModelAnimationTagResource.AnimationCompressionFormats.Type5)
                                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.KeyframeType5>(dataContext));
                                    else
                                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.Keyframe>(dataContext));
                            ...
                            case ModelAnimationTagResource.AnimationCompressionFormats.Type8:
                                var Format8 = blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.Format8>(dataContext);
                                ...
                                for (int nodeIndex = 0; nodeIndex < codec.RotationNodeCount; nodeIndex++)
                                    for (int frameIndex = 0; frameIndex < member.FrameCount; frameIndex++)
                                        CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.RotationFrameFloat>(dataContext));
                        ...
                        switch (member.MovementDataType)
                        {
                            case ModelAnimationTagResource.GroupMemberMovementDataType.dx_dy:
                                for (int i = 0; i < member.FrameCount; i++)
                                    CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.FrameInfoDxDy>(dataContext));
                                break;

                            case ModelAnimationTagResource.GroupMemberMovementDataType.dx_dy_dyaw:
                                for (int i = 0; i < member.FrameCount; i++)
                                    CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.FrameInfoDxDyDyaw>(dataContext));
                                break;

                            case ModelAnimationTagResource.GroupMemberMovementDataType.dx_dy_dz_dyaw: // NOT VERIFIED
                                for (int i = 0; i < member.FrameCount; i++)
                                    CacheContext.Serializer.Serialize(dataContext, blam_cache.Deserializer.Deserialize<ModelAnimationTagResource.GroupMember.FrameInfoDxDyDzDyaw>(dataContext));
                                break;
