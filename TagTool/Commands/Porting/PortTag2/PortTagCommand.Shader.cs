...
namespace TagTool.Commands.Porting
{
    partial class PortTag2Command
    {
        ...
        private RenderMethod ConvertRenderMethod(CacheFile blam_cache, List<ConvertTagTask> tag_conversion_tasks, Stream cacheStream, Dictionary<ResourceLocation, Stream> resourceStreams, RenderMethod finalRm, string blamTagName)
        {
            ...
            var bmRmt2Instance = blam_cache.IndexItems.Find(x => x.ID == finalRm.ShaderProperties[0].Template.Index);
            var blamContext = new CacheSerializationContext(ref blam_cache, bmRmt2Instance);
            var bmRmt2 = blam_cache.Deserializer.Deserialize<RenderMethodTemplate>(blamContext);

            // Get a simple list of H3 bitmaps and arguments names
            foreach (var a in bmRmt2.ShaderMaps)
                bmMaps.Add(blam_cache.Strings.GetItemByID(a.Name.Index));
            foreach (var a in bmRmt2.Arguments)
                bmArgs.Add(blam_cache.Strings.GetItemByID(a.Name.Index));

            // Find a HO equivalent rmt2
            var edRmt2Instance = FixRmt2Reference(blam_cache, cacheStream, CacheContext, bmRmt2Instance, bmRmt2, bmMaps, bmArgs);
            ...
            foreach (var a in edMaps)
            {
                ...
                {
                    //TODO: Clean this up, this created the parent object and then runs the async job to assign the output
                    var parent = new RenderMethod.ShaderProperty.ShaderMap
                    {
                        //Bitmap = bitmap
                    };
                    var fieldinfo = typeof(RenderMethod.ShaderProperty.ShaderMap).GetField("Bitmap");
                    var legacy_tag = ParseLegacyTag(blam_cache, $"{newBitmap}.bitm")[0];
                    CreateConvertTagJob(cacheStream, resourceStreams, legacy_tag, parent, fieldinfo);

                    //bitmap = ConvertTag(tag_conversion_tasks, cacheStream, resourceStreams, );
                }
            ...
            FixRmdfTagRef(blam_cache, finalRm);

            FixFunctions(blam_cache, tag_conversion_tasks, cacheStream, resourceStreams, blam_cache, CacheContext, finalRm, edRmt2, bmRmt2);
            ...
            foreach (var a in finalRm.ShaderProperties[0].ShaderMaps)
            {
                ...
                catch
                {
                    ...
                    var fieldinfo = a.GetType().GetField("Bitmap");
                    var legacy_tag = ParseLegacyTag(blam_cache, $"{defaultBitmap}.bitm")[0];
                    CreateConvertTagJob(cacheStream, resourceStreams, legacy_tag, a, fieldinfo);
                }
            }
        ...
        private List<ShaderTemplateItem> CollectRmt2Info(CacheFile blam_cache, Stream cacheStream, CacheFile.IndexItem bmRmt2Instance, List<string> bmMaps, List<string> bmArgs)
        {
            var edRmt2BestStats = new List<ShaderTemplateItem>();

            var bmRmt2Context = new CacheSerializationContext(ref blam_cache, bmRmt2Instance);
            var bmRmt2 = blam_cache.Deserializer.Deserialize<RenderMethodTemplate>(bmRmt2Context);
            var bmPixlContext = new CacheSerializationContext(ref blam_cache, blam_cache.IndexItems.GetItemByID(bmRmt2.PixelShader.Index));
            var bmPixl = blam_cache.Deserializer.Deserialize<PixelShader>(bmPixlContext);
        ...
        private CachedTagInstance FindEquivalentRmt2(CacheFile blam_cache, Stream cacheStream, CacheFile.IndexItem blamRmt2Tag, RenderMethodTemplate blamRmt2Definition, List<string> bmMaps, List<string> bmArgs)
        {
            ...
            edRmt2BestStats = CollectRmt2Info(blam_cache, cacheStream, blamRmt2Tag, bmMaps, bmArgs);
        ...
        private CachedTagInstance FixRmt2Reference(CacheFile blam_cache, Stream cacheStream, HaloOnlineCacheContext CacheContext, CacheFile.IndexItem blamRmt2Tag, RenderMethodTemplate blamRmt2Definition, List<string> bmMaps, List<string> bmArgs)
        {
			...
			return FindEquivalentRmt2(blam_cache, cacheStream, blamRmt2Tag, blamRmt2Definition, bmMaps, bmArgs);
		}
		...
		private void FixRmdfTagRef(CacheFile blam_cache, RenderMethod finalRm)
		{
			// Set rmdf
			var rmdfName = blam_cache.IndexItems.Find(x => x.ID == finalRm.BaseRenderMethod.Index).Name;
		...
		private RenderMethod FixFunctions(CacheFile blam_cache, List<ConvertTagTask> tag_conversion_tasks, Stream cacheStream, Dictionary<ResourceLocation, Stream> resourceStreams, CacheFile blamCache, HaloOnlineCacheContext CacheContext, RenderMethod finalRm, RenderMethodTemplate edRmt2, RenderMethodTemplate bmRmt2)
        {
            ...
            {
                a.Name = (StringId)ConvertData(blam_cache, tag_conversion_tasks, cacheStream, resourceStreams, a.Name, null, null);
            ...
            var blamContext = new CacheSerializationContext(ref blam_cache, blamCache.IndexItems.Find(x => x.ID == bmRmt2.PixelShader.Index));
            var bmPixl = blam_cache.Deserializer.Deserialize<PixelShader>(blamContext);
            ...
            foreach (var a in bmPixl.DrawModes)
            {
                ...
                {
                    var ParameterName = blam_cache.Strings.GetItemByID(b.ParameterName.Index);
